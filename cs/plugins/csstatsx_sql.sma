...
public plugin_precache()
{
	...
	*/
	...
	/*
	* id do servidor (server_name na tabela). Define quais linhas o servidor grava.
	* Valor por servidor, ex.: csstats_sql_server "main" / "frag"
	*/
	cvar[CVAR_SQL_SERVER] = register_cvar("csstats_sql_server","")
...
#endif
{
	...
	get_pcvar_string(cvar[CVAR_SQL_SERVER],server_name,charsmax(server_name))
	...
	if(get_pcvar_num(cvar[CVAR_SQL_CREATE_DB]))
	{
		...
		if(strcmp(type,"mysql") == 0)
		{
			...
			que_len += formatex(query[que_len],charsmax(query) - que_len,"`%s` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,\
				...
				`%s` varchar(32) NOT NULL DEFAULT 'main',\
					PRIMARY KEY (%s),\
					UNIQUE KEY `server_steamid` (`%s`(16),`%s`(16)),\
					KEY `%s` (`%s`(16)),\
					KEY `%s` (`%s`(16)),\
					KEY `%s` (`%s`)\
				...
				"server_name",
				row_names[ROW_ID],
				"server_name",row_names[ROW_STEAMID],
...
DB_LoadPlayerData(id)
{
	// server.cfg pode executar depois do plugin_cfg no boot; re-lê o cvar
	// para garantir o filtro/INSERT do servidor correto
	get_pcvar_string(cvar[CVAR_SQL_SERVER],server_name,charsmax(server_name))
	...
	// filtra por servidor: cada servidor grava apenas suas linhas
	if(server_name[0])
	{
		len += formatex(query[len],charsmax(query)-len," AND `server_name` = '%s'",
			server_name
		)
	}
...
{
	// re-lê o cvar do servidor (seguro contra ordem de exec do server.cfg no boot)
	get_pcvar_string(cvar[CVAR_SQL_SERVER],server_name,charsmax(server_name))
	...
	switch(player_data[id][PLAYER_LOADSTATE])
	{
		...
		case LOAD_NEW: // запрос на добавление новой записи
		{
			...
			formatex(query,charsmax(query),"INSERT INTO `%s` \
							(`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`%s`,`server_name`)\
							VALUES('%s','%s','%s','%.2f','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d','%d',CURRENT_TIMESTAMP,'%s')\
							",tbl_name,
...
DB_QueryBuildScore(sql_que[] = "",sql_que_len = 0,bool:only_rows = falos,overide_order = 0)
{
	...
	else
	{
		...
		filter[0] = 0
		if(server_name[0])
			formatex(filter,charsmax(filter)," AND `server_name` = '%s'",server_name)
		
		switch(overide_order ? overide_order : get_pcvar_num(cvar[CVAR_RANKFORMULA]))
		{
			case 1: return formatex(sql_que,sql_que_len,"SELECT COUNT(*) FROM %s WHERE (kills)>=(a.kills)%s",tbl_name,filter)
			case 2: return formatex(sql_que,sql_que_len,"SELECT COUNT(*) FROM %s WHERE (kills+hs)>=(a.kills+a.hs)%s",tbl_name,filter)
			case 3: return formatex(sql_que,sql_que_len,"SELECT COUNT(*) FROM %s WHERE (skill)>=(a.skill)%s",tbl_name,filter)
			case 4: return formatex(sql_que,sql_que_len,"SELECT COUNT(*) FROM %s WHERE (connection_time)>=(a.connection_time)%s",tbl_name,filter)
			default: return formatex(sql_que,sql_que_len,"SELECT COUNT(*) FROM %s WHERE (kills-deaths-tks)>=(a.kills-a.deaths-a.tks)%s",tbl_name,filter)
		}
...
{
	if(server_name[0])
		return formatex(sql_que,sql_que_len,"SELECT COUNT(*) FROM %s WHERE `server_name` = '%s'",tbl_name,server_name)
...
DB_QueryBuildGetstats(query[],query_max,len = 0,index,index_count = 2,overide_order = 0)
{
	...
	filter[0] = 0
	if(server_name[0])
		formatex(filter,charsmax(filter)," WHERE `server_name` = '%s'",server_name)
	len += formatex(query[len],query_max-len," FROM `%s` as `a`%s ORDER BY `rank` DESC LIMIT %d,%d",
		tbl_name,filter,index,index_count
	)
