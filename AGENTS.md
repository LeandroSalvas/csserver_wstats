...
- Comandos (prefixo `!`, só quem tem role/ID autorizado): `!status`, `!servidores`, `!stack`, `!start|!stop|!restart <id>`, `!rcon <id> <comando>`, `!ajuda`. Respostas truncadas em ~1900 chars (limite do Discord). Mutações também disparam `sendAlert` (mesmo webhook dos alertas).
