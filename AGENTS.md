...
- **Slots & bots**: `maxplayers` in `servers.list` = **visible** slots (even: 8/16/24/32; `setup.sh` offers a menu, `init` validates even 2..32). The engine maxplayers is `visible + 1` (hidden slot reserved for the HLTV via `sv_visiblemaxplayers <visible>` + AMXX plugin `slots_reserve.amxx`, which kicks the next non-HLTV occupant with `slots_reserve_msg`). **Teto 32**: CS 1.6 clamps `+maxplayers` to 32 (33 reverts to 32), so a 32-slot server has no hidden slot — the plugin reserves the 32nd slot. `servers.sh compose` computes `MAXPLAYERS` with a 32 cap. GameDig (API `/servers`) reads `sv_visiblemaxplayers`, so the API already reports the visible number. **Bot floor**: `cs16/Dockerfile` patches `podbot.cfg` to `pb_minbots 2` (file is CRLF — sed must match `^pb_minbots 0\r$`) so the HLTV relay connecting no longer kicks the last bot. PodBot `pb_maxbots 32` auto-refills bots as humans leave; `pb removebots` (via RCON) resets to the 2-bot floor.
...
- **HLTV relay can go "silent"** (process alive + connected to the game server but no data to spectators): the watchdog/teardown unlock the frozen spectator; the **operational recovery is `scripts/watch-mudo.sh`** (cron every minute), which detects mudo by **game-packet rate** — the proxy logs `recving N bytes` per UDP packet; the HLTV keepalive is exactly 48 bytes (~every 2s), a healthy stream is ~99% game packets (≠48B) at ~10-20/s, while mudo/degraded is keepalive-only (or a ~0.3-0.5/s trickle). It kicks the HLTV via RCON when there are **fewer than 45 ≠48B packets in 90s** and someone has been watching (bridge open in the last 10 min) — a relay `docker restart` was proven NOT to fix the mudo (the relay reconnected but stayed silent), while the kick forces a fresh netchan handshake (~20s recovery). If the kick doesn't land it escalates to `docker restart cs16-watch-hltv`; after 3 consecutive episodes it logs a warning to restart the game server (`docker compose restart cs16`; HLTV reconnects in ~20s), the definitive fix. Also fix in the client: `sendto()` is guarded (never throws `InvalidStateError` on a closed channel, which used to abort the WASM frame — see submodule `PATCHES.md` §11).
...
- Deploy config (not in the fork) lives in `config/watch/`: `hltv.cfg` (relay connects to the primary server at host `27015`, fixed) and `start-hltv.sh` (auto-restart loop, pidfile for healthcheck, crash log to `/home/cs16/watch_logs/last_hltv_crash.txt` → mounted `./live/watch`).
- Networking: `watch-main` listens on host port `27018` (TCP, page + `/websocket` signaling) and WebRTC UDP in `27019-27050` (`WATCH_UDP_PORT_RANGE`), all `network_mode: host`. It resolves the browser's mDNS `.local` candidates over the LAN (same-LAN viewers connect via the host candidate `192.168.15.54`) and discovers the public IP via STUN (`stun.l.google.com`) per connection — no `WATCH_PUBLIC_IP` hardcoded anymore (a stale public IP used to break external/mobile viewers when the ISP's dynamic IP changed). Ports `27018` + `27019-27050` (TCP/UDP) must be forwarded to this host on the router.
- TLS exposure: swag (separate stack in `~/duckdns`) serves the spectator on `https://zueiracstrike.duckdns.org:4445` via `proxy-confs/zueiracstrike-watch.subdomain.conf` → `http://192.168.15.54:27018` (WebSocket upgrade via swag `proxy.conf`). The client uses `window.location.host`, so it becomes `wss://...:4445/websocket` automatically. Port `4445` must be forwarded too. Frontend button link: `SPECTATOR_URL` in `web/common.js`.
- Healthchecks: `watch-main` hits `/health` (image includes `wget`); `watch-hltv` checks the HLTV pidfile. `watch.sh status` shows health + last HLTV crash time.
