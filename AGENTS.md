...
- `.env` sets `COMPOSE_FILE=docker-compose.yml:docker-compose.servers.yml`, so plain `docker compose ...` works after generation. The spectator compose is NOT in `COMPOSE_FILE` — it's opt-in and only used by `watch.sh` (which passes `-f` explicitly).
...
- Networking: every `watch-main-<id>` listens on host TCP `27200+i` (page + `/websocket` signaling) and WebRTC UDP in its `27300+(i*64)..+63` range, all `network_mode: host`. It resolves the browser's mDNS `.local` candidates over the LAN (same-LAN viewers connect via the host candidate `192.168.15.54`) and discovers the public IP via STUN (`stun.l.google.com`) per connection — no `WATCH_PUBLIC_IP` hardcoded anymore (a stale public IP used to break external/mobile viewers when the ISP's dynamic IP changed). Ports `27200-27202` (TCP) + `27300-27491` (UDP) + relays `27100-27102` (UDP) must be forwarded to this host on the router.
- TLS exposure: swag (separate stack in `~/duckdns`) serves each spectator on `https://zueiracstrike.duckdns.org:4445/<context>/` via `proxy-confs/zueiracstrike-watch.subdomain.conf` → `http://192.168.15.54:27200+i`. `servers.sh compose` writes the location blocks (`proxy_pass` WITHOUT trailing slash — the proxy keeps the `BASE_PATH` prefix) to `config/watch/swag-locations.conf.example` **and auto-syncs the live conf** (`apply_swag_locations`, called at the end of `cmd_compose`, so it covers `up`/`compose`/`provision`/`unprovision`; manual re-sync via `servers.sh swag-sync`): it rewrites only the region between `# BEGIN servers.sh swag locations` / `# END servers.sh swag locations` (first run migrates the legacy blocks), takes a one-time `${conf}.bak`, and restarts the swag container (`SWAG_CONTAINER`, default `swag`) only when the file changed AND `docker exec swag nginx -t` passes. Blocks derive from `servers.list`, so removing a server drops its location too. The swag conf is outside the repo (`~/duckdns/swag/config/nginx/proxy-confs/`); the api's disposable provision container (add/remove via UI) mounts the repo's PARENT dir at the same host path (`serverManager.js` `runProvision`, sempre existe — sem risco de o docker criar dirs vazios) e `servers.sh` deriva o conf vivo de `ROOT` (`dirname $ROOT/duckdns/...`); sem `~/duckdns` no host ele avisa e segue. O `apply_swag_locations` grava o arquivo via `mktemp`+`mv` **só quando o conteúdo muda** e restaura dono/modo originais (`stat`/`chown`/`chmod`) — se rodar do container de provisionamento (root) sem isso, o conf viraria root:root 0600 e o host não conseguiria mais ler (já aconteceu). **Atenção swag/cert**: o `init-certbot-config/run` do swag (imagem `lscr.io/linuxserver/swag:latest` de ~2026) tem um check "old LE root" quebra­do — testa se o emissor do 1º cert do `chain.pem` contém "ISRG Root X", mas a cadeia atual do Let's Encrypt (YE2→Root YE) faz o check falhar SEMPRE, então TODO restart revoga o cert e reemite, queimando a cota do LE (5 certs/168h p/ o mesmo domínio → swag fica sem cert e unhealthy por horas). Já foi contornado **editando o script dentro do container** (removido o `!` do check; persiste em `docker restart`, mas se perde num `docker compose` recreate — nesse caso repetir o contorno ou atualizar o swag). Backup dos certs: `~/duckdns/backups/config-*.tar.gz` (restaurar `etc/letsencrypt/{live,archive,renewal}` resolve sem queimar cota). The client uses `window.location.host`, so it becomes `wss://...:4445/<context>/websocket` automatically. **Router forwarding must grow with `servers.list`**: per index `i` (0-based) forward relay UDP `27100+i`, watch-main TCP `27200+i`, and WebRTC ICE UDP `27300+(i*64)..+(i*64)+63` — with 4 servers today: `27100-27103`, `27200-27203`, `27300-27555`, plus TLS `4445`. Frontend: `/servers` returns `spectatorUrl` per server; the "Assistir" button/iframe on `cstv.html`/`live.html` follows the server selector (`data-watch-link`, `data-spectator-frame`), with `SPECTATOR_URL` as static fallback in `web/common.js`.
