...
- `config/servers.list` is the single source of truth (first line = primary `main`). The primary always uses host port `27015` (fixed in `docker-compose.yml` base; `compose` errors out if the list says otherwise). Server ids must match `^[a-z0-9][a-z0-9_-]*$`. Format: `id name host_port map maxplayers rotate context mode cstv` — `context` is the optional 7th column (web-spectator URL path, `^[a-z0-9]+$`, unique; default = slugified `name`, e.g. `Zueira` → `zueira`). `mode` is the 8th column (`standard|zombies|csdm|surf|gungame`, default `standard`). `cstv` is the 9th column (`yes|no`, default `no`) — when `no`, no watch containers (relay/proxy) are generated for that server.
- `./scripts/setup.sh` — interactive wizard: asks how many servers/names, whether to rotate maps (and which maps, from the image's available pool), whether to enable CSTV (web-spectator), writes `servers.list` (backing up the old one to `servers.list.bak`), generates configs, and starts the stack (flags: `--no-up`, `--yes`).
...
- **Multi-server**: one `watch-main-<id>` (proxy WebRTC->UDP) + `watch-hltv-<id>` (HLTV relay) per server in `servers.list` **where `cstv=yes`**, all carrying `profiles: ["watch"]`, so `servers.sh up` / plain `docker compose up` never starts them. `servers.sh compose` / `servers.sh init` / `watch.sh up` all gate watch config generation on `cstv=yes` — servers with `cstv=no` get no relay/proxy containers, no `config/watch/<id>/` dir, no swag location block. `reconcile_watch_ports()` detects stale env vars (wrong `LISTEN_PORT`/`GAME_SERVER`) and force-recreates. Manage with `./scripts/watch.sh up|down|build|ps|status|logs|restart|backup|restore`. `watch.sh up` builds `cs16_stats:local` if missing (relay image) and always rebuilds the proxy image `csserver_wstats-watch-main` (only `watch-main-<first>` carries the `build:` in compose; the rest reuse the image). **Atenção `--remove-orphans`**: o `WATCH_COMPOSE` do `watch.sh` inclui `docker-compose.duckdns.yml` **se presente** — sem esse arquivo, o `up --remove-orphans` remove o `swag`/`duckdns` do projeto (órfãos da config parcial) e derruba o site/HTTPS (aconteceu em 2026-08-14). O `watch.sh down` usa `down $(watch_services)` (só o espectador); nunca `down` sem lista (derrubaria TODO o projeto).
...
### Alertas via webhook (Discord) — `api/lib/alerts.js`, `api/lib/serverCtx.js`
- `ALERT_WEBHOOK_URL` (`.env`, gitignored) aponta para um webhook **incoming do Discord** (canal específico). `api/lib/alerts.js` centraliza `sendAlert()` (payload Discord `{content}` ou Slack/Genérico `{text}`), `pushAlertEvent()` e `alertEvents` (log dos últimos 20, exposto em `GET /alerts` com `configured`, `currentStatus`, `stackStatus` e `events`).
- Eventos atuais (tipos em `events[].type`): **servidores de jogo** `online`/`offline` (GameDig, `checkServerAlerts`), **stack** `stack-up`/`stack-down` (`checkStackAlerts`) e **usuários** `user-pending`/`user-approved`/`user-rejected`. Mensagens Discord com emoji 🟢/🔴/🆕/⛔.
- `checkStackAlerts` (mesmo intervalo de 30s, `index.js` roda `checkServerAlerts` + `checkStackAlerts` em `Promise.allSettled`): lista os containers do projeto via docker.sock (`label com.docker.compose.project=$STACK_PROJECT`, default `csserver_wstats`) e alerta na **transição** de estado. `up` = `State=running` sem `(unhealthy)` (healthcheck); `(health: starting)` conta como up. **Serviços `cs16*` ficam de fora** (já têm alerta próprio via GameDig) — db/redis/api/web/prometheus/grafana/exporters/node-exporter/cadvisor/swag/duckdns e espectador `watch-*` entram, com nome amigável (`STACK_SERVICE_NAMES`). Docker indisponível → loga e ignora (sem crash); estado reseta no restart da api (primeiro poll só semeia, sem alerta).

### Bot Discord (gerencia a stack) — `api/lib/discordBot.js`
- Roda **no processo da API** (`startDiscordBot()` no listen; `destroyDiscordBot()` no shutdown gracioso) e chama os módulos internos **direto** (sem HTTP/CSRF): `serverManager` para start/stop/restart, `runRconCommand` para RCON, `stackHealthState` para o estado da stack, `queryServer`/`findServer` para o estado dos jogos.
- Config (`.env`): `DISCORD_BOT_TOKEN` (crie um application em `discord.com/developers` → Bot; **ative o intent privilegiado MESSAGE CONTENT** e convide o bot ao servidor), `DISCORD_ALLOWED_ROLE_IDS` e/ou `DISCORD_ALLOWED_USER_IDS` (listas separadas por vírgula). Sem token ou sem permissões → desabilitado sem crash (mesmo padrão do webhook). Os três são repassados ao container no `docker-compose.yml` (bloco env da api).
- Comandos (prefixo `!`, só quem tem role/ID autorizado): `!status`, `!servidores`, `!stack`, `!start|!stop|!restart <id>`, `!rcon <id> <comando>`, `!changelevel <id> <mapa>`, `!mapas`, `!player <nome|steamid>`, `!logs <id|serviço> [n]`, `!ps`, `!watch [id]`, `!uptime`, `!ajuda`. `!logs`/`!ps`/`!watch`/`!mapas` usam o CLI do docker no processo da API (mesmo socket do `serverManager`); `!player` consulta `csstats` direto (mesmo SQL de `top.js`, filtro `steamid NOT LIKE 'BOT%'`); `!watch` lê `live/watch/<id>/` (`last_hltv_crash.txt`, `.mudo_count`) via `SERVER_REPO_DIR`. Respostas truncadas em ~1900 chars (limite do Discord). Mutações também disparam `sendAlert` (mesmo webhook dos alertas).
- **Volume do espectador é impossível pelo bot**: o áudio é local do navegador do espectador (Web Audio), nada no servidor o controla.
