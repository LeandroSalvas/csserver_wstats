...
- Networking: `watch-main` listens on host port `27018` (TCP, page + `/websocket` signaling) and WebRTC UDP in `27019-27050` (`WATCH_UDP_PORT_RANGE`), all `network_mode: host` because it announces `WATCH_PUBLIC_IP` as ICE host candidate and resolves the browser's mDNS `.local` candidates over the LAN. Ports `27018` + `27019-27050` (TCP/UDP) must be forwarded to this host on the router.
