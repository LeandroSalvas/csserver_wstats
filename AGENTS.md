...
- `config/prometheus.yml` tem 4 jobs: `cs16-api` (com Basic Auth), `nginx` (`nginx-exporter:9113`), `node` (`host.docker.internal:9100`) e `cadvisor` (`cadvisor:8080`). Edições no yml entram com `curl -X POST 127.0.0.1:9090/-/reload` (Prometheus já roda com `--web.enable-lifecycle`).
- Dashboard `cs16-infra.json` (provisionado por arquivo; incremente `version` no JSON para o Grafana recarregar em ≤60s) tem 3 seções: **API e Infra** (API/nginx), **Host** (CPU %, memória %, rede % da interface `$iface` — usa `node_network_speed_bytes`, variável custom `iface` default `eno1`) e **Stack — containers** (CPU/memória/rede por container e proporções, filtrando `container_label_com_docker_compose_project="csserver_wstats"` — o cAdvisor vê TODOS os containers do host, inclusive plex/swag/duckdns de outros projetos; o label de projeto os exclui). Containers `network_mode: host` (watch-main/hltv) reportam contadores do namespace do host no cAdvisor — os painéis de rede são confiáveis para os demais serviços.
