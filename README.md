...
- Com a aba oculta: shim de `requestAnimationFrame` mantém o loop do engine vivo e o backlog de `net.incoming` foi elevado para `maxPackets: 8192` (~5,7 min a 24fps) — esconder a aba por pouco tempo "avança rápido" até o ao-vivo em vez de congelar; mais de 60s oculto reconecta.
...
- Hidden tab: a `requestAnimationFrame` shim keeps the engine loop alive and the `net.incoming` backlog is raised to `maxPackets: 8192` (~5.7min at 24fps) — short hides fast-forward to live instead of freezing; hides over 60s reconnect.
