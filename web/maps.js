async function loadMaps() {
  ...
  try {
    const maps = await fetchJson(`/maps${serverQuery()}`)
    clearStatus()
    ...
    maps.forEach((m, i) => {
      ...
      row.innerHTML = `
        <td>${i + 1}</td>
        <td><a href="map.html?map=${encodeURIComponent(m.map)}${serverParam()}">${escapeHtml(m.map)}</a></td>
        <td>${m.snapshots}</td>
      `
...
document.addEventListener('server-change', loadMaps)
